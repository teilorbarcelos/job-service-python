...
from src.modules.audit.audit_context import audit_json_dumps
...
async def test_should_log_audit_with_details(client, session):
    ...
    assert audit_log_post.function_name is not None
...
def test_audit_json_dumps_decimal():
    data = {"val": Decimal("10.50"), "date": datetime(2023, 1, 1)}
    res = audit_json_dumps(data)
    assert "10.5" in res
